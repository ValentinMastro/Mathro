<script lang="ts">
    ...
    onMount(async () => {
        ...
        async function remplir_tableau(graine: number) {
            // Addition
            await exercice(4, 0, "qcm", 6, 1, graine)
            // Soustraction
            await exercice(4, 4, "qcm", 6, 2, graine)
            // Multiplication
            await exercice(4, 8, "qcm", 6, 3, graine)
            // Addition et Multiplication
            await exercice(4, 12, "qcm", 6, 4, graine)
            // Carre
            await exercice(4, 16, "qcm", 6, 5, graine)
    ...
    {#each tableau as ligne, numero_ligne }
        <tr>
            ...
            {#each ligne as data, i}
                {#if (i == 0 && numero_ligne < 8)}
                    <td class="enonce">{data}</td>
                {:else if (i==0 && numero_ligne >= 8)}
