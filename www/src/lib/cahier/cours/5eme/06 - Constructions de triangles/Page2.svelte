...
<Contenu>
	...
	<LigneVide />
	<Exemple>
		Tracer {@html math('\\text{ABC}')} tel que {@html math('\\text{AB} = 5\\,\\text{cm}')}, {@html math('\\text{AC} = 4\\,\\text{cm}')} et {@html math(
			'\\text{BC} = 3\\,\\text{cm}'
		)}.
	</Exemple>
	<Schema lignes={10} aspectRatioSVG={0.8}>
		{#snippet svg()}
			...
			<circle cx={200} cy={800} r={300} fill="none" stroke="black" stroke-width={8} stroke-dasharray="10,10" />
			<!-- arc de centre (200+500,800) et de rayon 400 -->
			<circle cx={200 + 500} cy={800} r={400} fill="none" stroke="black" stroke-width={8} stroke-dasharray="10,10" />
			<!-- côtés -->
			<line
				x1={200}
				y1={800}
				x2={200 + Math.cos(Math.acos(3 / 5)) * 300}
				y2={800 - Math.sin(Math.acos(3 / 5)) * 300}
				stroke="black"
				stroke-width={8}
			/>
			<line
				x1={200 + 500}
				y1={800}
				x2={200 + 500 - Math.cos(Math.acos(4 / 5)) * 400}
				y2={800 - Math.sin(Math.acos(4 / 5)) * 400}
				stroke="black"
				stroke-width={8}
			/>
			<!-- points -->
