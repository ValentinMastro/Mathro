<script lang="ts">
	...
	let C = $state(new Point2D(300, 200));
	let D = $state(new Point2D(1200, 400));
	let E = $state(new Point2D(900, 900));
...
<Contenu apres_un_titre>
	...
	<Schéma lignes={10} aspectRatioSVG={2}>
		{#snippet svg()}
			<Polygone points={[C, D, E]} afficher_points {type} distance_nom={70} />
			{#if étape >= 1}
