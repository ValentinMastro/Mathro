...
<Contenu>
	...
	<Schéma lignes={5} aspectRatioSVG={2}>
		{#snippet svg()}
			...
			<Droite passantPar={[Aex, HA_ex]} stroke="red" stroke-width={10} />
			<AngleDroit points={[Aex, HA_ex, Cex]} taille={80} />
			<Point point={HA_ex} nom="H" dx={80} dy={150} font-size={120} />
		{/snippet}
	</Schéma>
	<Notation lignes={3}>
		<Item>{@html math('(\\text{AH})')} est la hauteur du côté {@html math('[\\text{BC}]')}</Item>
		<Item>{@html math('(\\text{AH})')} est la hauteur relative à {@html math('[\\text{BC}]')}</Item>
