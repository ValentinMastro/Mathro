<script lang="ts">
	import { Contenu, SousPartie, SousSousPartie } from '$lib/cahier/composants/de_chapitrage/*';
...
<Contenu>
	...
	<Formule lignes={2}>
		Si le cylindre a une hauteur {@html math(`h`)} et un rayon de base {@html math(`r`)}, alors {@html math(
	...
	<Formule lignes={2}>
		Si le cône a une hauteur {@html math(`h`)} et un rayon de base {@html math(`r`)}, alors {@html math(
			`V_{\\text{cone}} = \\frac{1}{3} \\times \\pi r^2 \\times h`
		)}.
	</Formule>
