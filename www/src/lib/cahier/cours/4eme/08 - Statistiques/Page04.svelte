...
<Contenu>
	<Methode lignes={6}>
		...
		<Item bullet="1. " bullet_color="black">On ordonne les valeurs de la série par ordre croissant.</Item>
		...
		<Item bullet="3. " bullet_color="black" lignes={2}>
			Si l'effectif total est pair, la médiane est la moyenne des deux valeurs du milieu (la {@html math(`\\frac{\\text{N}}{2}`)}ème et la {@html math(
				`(\\frac{\\text{N}}{2}+1)`
			)}ème valeurs).
		</Item>
	</Methode>
	<Exemples lignes={13}>
		<Item lignes={7}>
			...
			<Item bullet="➜" lignes={2}>
				On trie les valeurs dans l'ordre croissant : {@html math(
					`\\left\\{ ${serie_1
						.slice()
						.sort((a, b) => a - b)
						.join('~;~')}  \\right\\}`
