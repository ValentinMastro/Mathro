<script lang="ts">
    ...
    function cos(angle: number) { return Math.cos(angle * Math.PI / 180); }
    function sin(angle: number) { return Math.sin(angle * Math.PI / 180); }
    ...
    <SousPartie numero={2} titre="Diagrammes circulaires" />
        ...
        <Exemples lignes={3} addStyle="margin-bottom: -0.2ex;">
            <table class="donnees" style="--hauteur: {$hauteur_ligne_cahier}px; --taille-texte: {$taille_texte}px;">
                <caption>
                    Population de {villes[index_ville].meta.ville} ({villes[index_ville].meta.code_insee}) 
                    en {villes[index_ville].meta.annee} par grandes tranches d'âge
                </caption>
                <tbody>
                    <tr>
                        <th>Tranche d'âge</th>
                ...
                {#each Object.keys(villes[index_ville].donnees) as tranche, index}
                    {#if tranche != "TOTAL"}
                        <path 
                            ...
                            A 350 350 0 {angle(index) > 180 ? 1 : 0} 0 {350 + 350*cos(angle_cumule(index+1))} {500 + 350*sin(angle_cumule(index+1))}
                            Z"
