...
<Contenu>
	...
	<Propriete lignes={3}>
		Si deux fractions sont égales, alors on peut multiplier le numérateur et le dénominateur de l'une par un même nombre non nul pour obtenir l'autre
		fraction.
	</Propriete>
	<Exemples lignes={4}>
		<Item lignes={2}>
			{@html math(
				`\\dfrac{3}{4} \\begin{matrix} {\\color{red} \\rightarrow \\times 2} \\\\ {\\color{red} \\rightarrow \\times 2} \\end{matrix} = \\dfrac{6}{8}`
			)}.
		</Item>
		<Item lignes={2}>
			{@html math(
				`\\dfrac{2}{3} \\begin{matrix} {\\color{red} \\rightarrow \\times 4} \\\\ {\\color{red} \\rightarrow \\times 4} \\end{matrix} = \\dfrac{8}{12}`
			)}.
		</Item>
	</Exemples>
	<Definition lignes={2}>
