...
<script lang="ts">
	...
	let { centre, rayon1, rayon2, angle1: angle1_brut, angle2: angle2_brut, ...props }: Props = $props();
	...
	let petitRayonAngle1 = $derived({ x: centre.x + rayon1 * Math.cos(angle1), y: centre.y + rayon1 * Math.sin(angle1) });
	let petitRayonAngle2 = $derived({ x: centre.x + rayon1 * Math.cos(angle2), y: centre.y + rayon1 * Math.sin(angle2) });
	let grandRayonAngle1 = $derived({ x: centre.x + rayon2 * Math.cos(angle1), y: centre.y + rayon2 * Math.sin(angle1) });
	let grandRayonAngle2 = $derived({ x: centre.x + rayon2 * Math.cos(angle2), y: centre.y + rayon2 * Math.sin(angle2) });
